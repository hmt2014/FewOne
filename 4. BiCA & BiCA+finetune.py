...
class SentenceEncoder(nn.Module):
    ...
    def forward(self, X, params=None):
        ...
        if params == None:
            ...
            out = F.embedding(X, params['meta_learner.sentence_encoder.word_embeds.weight'])
            out = self.dropout(out)
...
class CompareAggregate_double(nn.Module):
    def __init__(self, config, sentence_encoder):
        super(CompareAggregate_double, self).__init__()
        """
    ...
    def forward(self, X, is_tuning=False):
        ...
        pred = torch.max(out2, dim=-1)[1].float()
        # print(logits)
        y = y.detach().cpu()
    ...
    def predict(self, X):
        ...
        out2, out_feature = self.pred(vq1, va1)
...
class Config():
    ...
    model = "compare_aggregate_double"
    optimizer = "adam"
    lr = 1e-3
    task_lr = 1e-1
    batch_size = 1
...
def train(model, optimizer, train_data, valid_data, test_data, config):
    ...
    pickle.dump([features, labels], open('BiCA_features.p', "wb"))
    _, _, features_tune, labels_tune = eval_epoch_finetune(saved_model, loader, config)
    pickle.dump([features_tune, labels_tune], open("BiCA_features_finetuned.p", "wb"))
...
def main():
    ...
    if config.model == "compare_aggregate_double":
        model = CompareAggregate_double(config, sentence_encoder)
